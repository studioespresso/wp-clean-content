...
if ( ! function_exists( 'clean_content_paging_nav' ) ) :
/**
 * Display navigation to next/previous set of posts when applicable.
 *
 * @return void
 */
function clean_content_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Posts navigation', 'clean-content' ); ?></h1>
		<div class="nav-links">

		<?php if ( get_next_posts_link() ) : ?>
			<span class="meta-nav next"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'clean-content' ) ); ?></span>
		<?php endif; ?>
			<?php if ( get_previous_posts_link() ) : ?>
				<span class="meta-nav previous"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'clean-content' ) ); ?></span>
			<?php endif; ?>
...
if ( ! function_exists( 'clean_content_post_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 *
 * @return void
 */
function clean_content_post_nav() {
...
if ( ! function_exists( 'clean_content_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function clean_content_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'clean-content' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'clean-content' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
 ...
 */
function clean_content_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so clean_content_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so clean_content_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in clean_content_categorized_blog.
 */
function clean_content_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'clean_content_category_transient_flusher' );
add_action( 'save_post',     'clean_content_category_transient_flusher' );
